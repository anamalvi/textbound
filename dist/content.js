(() => {
  ...
  // src/textMap.js
  var BLOCK_SELECTOR = "p, li, h1, h2, h3, h4, h5, h6, dd, blockquote, td, th, figcaption, label";
  ...
  var FLOAT_PROBE_SELECTOR = "img, figure, table, aside, div, span, picture";
  var MAX_BLOCKS = 80;
  var CELL = 10;
  var FLOAT_GAP = 6;
  function canvasFontFromStyle(style) {
    ...
    let family = (style.fontFamily || "Georgia").split(",").map((f) => f.trim().replace(/^["']|["']$/g, "")).find((f) => f && f !== "system-ui" && f !== "-apple-system" && f !== "BlinkMacSystemFont") || "Georgia";
  ...
  function getLayoutScope(el) {
    return el.closest(".mw-parser-output") || el.closest("article") || el.closest("main") || el.closest("#content") || el.parentElement;
  }
  ...
  function inkBoxesForLine(prepared, line, originX, originY, lineHeight, lineIndex) {
    ...
    if (line.end.graphemeIndex > 0 && line.end.segmentIndex < prepared.segments.length) {
      const seg = line.end.segmentIndex;
      const kind = prepared.kinds[seg];
      const fullW = prepared.widths[seg];
      if (kind === "text" && fullW > 0.5) {
        const text = prepared.segments[seg] || "";
        const ratio = text.length > 0 ? Math.min(1, line.end.graphemeIndex / [...text].length) : 1;
        boxes.push({
          x,
          y: inkY,
          w: fullW * ratio,
  ...
  var INCLUDE_ELEMENT_OBSTACLES = true;
  var ELEMENT_OBSTACLE_SELECTOR = "img, video, button, input, a";
  function collectElementObstacleBoxes(hostId = "any-page-snake-host") {
    const viewport = { width: window.innerWidth, height: window.innerHeight };
    const boxes = [];
  ...
  function buildTextOccupancy(hostId = "any-page-snake-host") {
    ...
    const elementBoxes = INCLUDE_ELEMENT_OBSTACLES ? collectElementObstacleBoxes(hostId) : [];
  ...
  `;
    ...
    <div class="cabinet">
      ...
      <div class="hint">navigate the gaps between words<br>arrows / wasd \xB7 p pause \xB7 esc quit</div>
    </div>
  `;
    ...
    function updateMapStatus() {
      if (!playing) {
        statusEl.textContent = showBoundaries && !mapReady ? "mapping boundaries\u2026" : "hit START to begin";
        return;
      }
      if (!mapReady) {
      ...
      const elementBit = elements ? ` \xB7 ${elements} elements` : "";
      statusEl.textContent = map.openCells.length < 20 ? "sparse text \u2014 mostly open field" : `${map.boxes.length} words${elementBit} \xB7 ${Math.round((1 - openPct) * 100)}% blocked`;
    }
    function rebuildMap() {
    ...
    function snakeNeedsReset(segments) {
      return segments.some(
        (s) => s.x < 0 || s.x >= map.cols || s.y < 0 || s.y >= map.rows || isBlocked(map, s.x, s.y)
      );
    }
    function normalizeGameStateAfterRebuild(prevMap) {
      ...
      if (food) {
        ...
        if (isBlocked(map, food.x, food.y) || snake.some((s) => s.x === food.x && s.y === food.y)) {
          placeFood();
        }
      }
    }
